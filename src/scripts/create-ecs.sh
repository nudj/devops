...
if [ $EXISTS -eq 0 ]; then
  ...
  aws cloudformation update-stack --stack-name $ENVIRONMENT-ecs --template-body "$template" --parameters ParameterKey=VPCStackName,ParameterValue=$STACK_NAME ParameterKey=EnvironmentType,ParameterValue=$ENVIRONMENT ParameterKey=DesiredCapacity,ParameterValue=1 ParameterKey=MaxSize,ParameterValue=1  ParameterKey=InstanceType,ParameterValue=t2.micro
  UPDATING=$?
